...
public class ProductService {
    ...
    public boolean productIndeAmount(ProductIndeAmountDomainRequest productIndeAmountDomainRequest) {
        ...
        ProductAddHistoryDomainRequest historyRequest = new ProductAddHistoryDomainRequest(productIndeAmountDomainRequest.productId(), productIndeAmountDomainRequest.productOptionId(), quantity, LocalDateTime.now());
    ...
    //
    public List<PopularProductDayEntity> popularProductDay(PopularProductDayDomainRequest popularProductDayDomainRequest) {
        return productRepository.popularProductDay(popularProductDayDomainRequest.startDate(), popularProductDayDomainRequest.endDate().plusDays(1));
    }

    //
    public List<PopularProductMonthEntity> popularProductMonth(PopularProductMonthDomainRequest popularProductMonthDomainRequest) {

        LocalDate startDate = LocalDate.of(
            popularProductMonthDomainRequest.startYearMonth().getYear(),
            popularProductMonthDomainRequest.startYearMonth().getMonthValue(),
            popularProductMonthDomainRequest.startYearMonth().lengthOfMonth()
        );

        LocalDate endDate = LocalDate.of(
            popularProductMonthDomainRequest.endYearMonth().getYear(),
            popularProductMonthDomainRequest.endYearMonth().getMonthValue(),
            popularProductMonthDomainRequest.endYearMonth().lengthOfMonth()
        );

        return productRepository.popularProductMonth(startDate.minusMonths(1), endDate);
    }
