...
//    @Query(value = "SELECT PRODUCT_ID, PRODUCT_NAME, PRODUCT_OPTION, PRODUCT_QUANTITY FROM PRODUCT_INFO p JOIN FETCH PRODUCT_OPTION_INFO WHERE p.PRODUCT_ID = :productId AND PRODUCT_OPTION_INFO = :productOptionName", nativeQuery = true)
//    ProductIndeAmountInfraEntity findByProductId(@Param("productId") Long productId, @Param("productOptionName") String productOptionName);
    ...
    ProductQuantityInfraEntity updateByProductId(@Param("productId") long productid, @Param("productOptionId") long productOptionId, @Param("productQuantity") int productQuantity);
}
